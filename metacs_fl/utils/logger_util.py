from logging import FileHandler, Formatter, getLevelName, Logger, LogRecord, StreamHandler
...
def load_logger(logging_settings: dict,
    ...
    if enable_logging:
        ...
        if log_to_file:
            ...
            file_handler = FlushFileHandler(filename=file_name, mode=file_mode, encoding=encoding, delay=False)
            file_handler.setLevel(logger.getEffectiveLevel())
            file_handler.setFormatter(formatter)
            logger.addHandler(file_handler)
        if log_to_console:
            console_handler = StreamHandler(__stdout__)
