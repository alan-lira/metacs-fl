            ...
            for metric_name in metrics_names:
                ...
                for n_available_clients in num_available_clients:
                    ...
                    if plotting_data:
                        ...
                        if metric_name == "max_training_elapsed_time":
                            total_elapsed_times = {}
                            for plotting_data_element in plotting_data:
                                client_selector = plotting_data_element["client_selector"]
                                max_training_elapsed_time = plotting_data_element["max_training_elapsed_time"]
                                if client_selector not in total_elapsed_times:
                                    total_elapsed_times.update({client_selector: max_training_elapsed_time})
                                else:
                                    total_elapsed_times[client_selector] += max_training_elapsed_time
                            message = "[TRAIN] Total elapsed times (in seconds): {0}".format(total_elapsed_times)
                            log_message(logger, message, "INFO")
                            # Set the 'y_data' value, if equals to 'Auto'.
                            if y_data == "Auto":
                        ...
                        # If the current metric name equals to 'sum_training_energy_cpu'...
                            ...
                            total_cpu_energy_consumptions = {}
                            for plotting_data_element in plotting_data:
                                client_selector = plotting_data_element["client_selector"]
                                sum_training_energy_cpu = plotting_data_element["sum_training_energy_cpu"]
                                if client_selector not in total_cpu_energy_consumptions:
                                    total_cpu_energy_consumptions.update({client_selector: sum_training_energy_cpu})
                                else:
                                    total_cpu_energy_consumptions[client_selector] += sum_training_energy_cpu
                            message = "[TRAIN] Total CPU energy consumptions (in Joules): {0}" \
                                      .format(total_cpu_energy_consumptions)
                            log_message(logger, message, "INFO")
                            # Set the 'y_data' value, if equals to 'Auto'.
                            if y_data == "Auto":
                        ...
                        # If the current metric name equals to 'weighted_mean_sparse_categorical_accuracy'...
                            ...
                            final_weighted_mean_accuracies = {}
                            for plotting_data_element in plotting_data:
                                client_selector = plotting_data_element["client_selector"]
                                weighted_mean_sparse_categorical_accuracy \
                                    = plotting_data_element["weighted_mean_sparse_categorical_accuracy"]
                                final_weighted_mean_accuracies.update({client_selector:
                                                                      weighted_mean_sparse_categorical_accuracy})
                            message = "[TRAIN] Final weighted mean accuracies: {0}" \
                                      .format(final_weighted_mean_accuracies)
                            log_message(logger, message, "INFO")
                            # Set the 'y_data' value, if equals to 'Auto'.
                            if y_data == "Auto":
                    ...
                    if plotting_data:
                        ...
                        # If the current metric name equals to 'weighted_mean_sparse_categorical_accuracy'...
                            ...
                            for plotting_data_element in plotting_data:
                                ...
                                final_weighted_mean_accuracies.update({client_selector:
                                                                      weighted_mean_sparse_categorical_accuracy})
                            message = "[TEST] Final weighted mean accuracies: {0}" \
                                      .format(final_weighted_mean_accuracies)
                            log_message(logger, message, "INFO")
                            # Set the 'y_data' value, if equals to 'Auto'.
                            if y_data == "Auto":
